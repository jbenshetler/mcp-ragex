#!/usr/bin/env python3
"""
Watchdog monitor for .mcpignore files

This module provides file system monitoring capabilities for the enhanced
ignore system, enabling hot reloading when .mcpignore files change.
"""
...
class IgnoreFileHandler(FileSystemEventHandler):
    """
    Watches for changes to .mcpignore files and notifies the IgnoreManager
    """
    ...
    def on_created(self, event: FileSystemEvent):
        """Handle creation of new .mcpignore files"""
    ...
    def on_modified(self, event: FileSystemEvent):
        """Handle modification of .mcpignore files"""
        if self._should_process_event(event):
    ...
    def on_deleted(self, event: FileSystemEvent):
        """Handle deletion of .mcpignore files"""
        if self._should_process_event(event):
    ...
    def on_moved(self, event: FileSystemEvent):
        """Handle moving of .mcpignore files"""
